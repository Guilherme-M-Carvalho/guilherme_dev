...
interface UpdateWorkOrderRepositoryProps {
    handleUpdate(work: WorkOrderEntity): Promise<WorkOrderEntity>
    handleValidationQuotation(id: number): Promise<QuotationEntity | null>
}

export class UpdateWorkOrderRepository implements UpdateWorkOrderRepositoryProps{
    async handleUpdate(work: WorkOrderEntity): Promise<WorkOrderEntity> {
        try {
